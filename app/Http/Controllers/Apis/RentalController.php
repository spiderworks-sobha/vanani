...
class RentalController extends Controller
{
    ...
    public function index(Request $request){
        try{
            ...
            $rentals = Rental::with(['featured_image', 'medias'])->where('status', 1);
    ...
    public function featured(Request $request){
        try{
            $rentals = Rental::with(['featured_image', 'home_image', 'featured_medias', 'medias'])->where('status', 1);
            if($tags = $request->tags){
                $rentals->whereHas('tags', function($query) use($tags){
                    $query->whereIn('rental_tag.tag_id', $tags);
                });
    ...
    public function details(Request $request, string $slug){
        try{
            $rental = Rental::with(['featured_image', 'banner_image', 'og_image', 'featured_video', 'features', 'tags', 'medias', 'faq', 'reviews'])->where('slug', $slug)->where('status', 1)->first();
            if($rental)
